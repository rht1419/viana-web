...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state?.hasError) {
      return (
        <div className="min-h-screen flex items-center justify-center" style={{ background: 'linear-gradient(180deg, #0a0214 0%, #110424 50%, #0b0516 100%)' }}>
          <div className="text-center p-8 max-w-md">
            ...
            <div className="flex flex-col gap-3 text-center">
              <h1 className="text-2xl font-bold heading-gradient">Something went wrong</h1>
              <p className="text-[color:var(--color-text-secondary)] text-base mx-auto">
                We encountered an unexpected error while processing your request.
              </p>
              {this.state?.error && (
                <pre className="mt-2 text-xs text-left whitespace-pre-wrap bg-white/5 border border-white/10 rounded-md p-3 overflow-auto max-h-48">
            ...
            <div className="flex justify-center items-center mt-6">
              <button
                ...
                className="text-[color:var(--color-background)] font-medium py-2 px-4 rounded flex items-center gap-2 transition-colors duration-200 shadow-sm"
